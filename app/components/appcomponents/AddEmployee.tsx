...
const AddEmployee = () => {
  ...
  const [newEmpValue, setNewEmpValue] = useState<{ id: string; firstName: string; lastName: string; contact: string ; email:string ; address: string; bloodgroup:string}>({
    id: uuidv4(),
    firstName: '',
    lastName: '',
    contact: '',
  ...
  const handleSubmitNewEmployee: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    setNewEmpValue({ id: '', firstName: '', lastName: '', contact:'',email:'',address:'',bloodgroup:''
   });
    setModalOpen(false);
    router.refresh();
  };
...
//     return <div>
//         <button onClick={() => setModalOpen(true)} className="btn btn-primary float-right">Add Emp<FaPlus className="ml-1" size={15} /></button>
//         <Modal modalOpen={modalOpen} setModalOpen={setModalOpen}>
//             <form onSubmit={handleSubmitNewEmployee}>
//                 <h3 className='font-bold text-lg'>Add New Employee</h3>
//                 <div className='modal-action'>
//                     <input
//                     value={newEmpValue}
//                     onChange={(e)=>setNewEmpValue(e.target.value)}
//                     type='text'
